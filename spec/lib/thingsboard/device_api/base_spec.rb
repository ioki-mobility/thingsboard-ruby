...
RSpec.describe Thingsboard::DeviceApi::Base do
  ...
  let(:expected_endpoint) { 'dummy_endpoint' }
  let(:expected_action) { 'dummy_action' }
  ...
  context 'as instantiated class with implementation' do
    ...
    context 'with a successful HTTP response' do
      ...
      it 'tracks the request with configured tracker' do
        expect(request_tracker).to receive(:call).with(expected_action)
        subject
      end
    ...
    context 'with a failing HTTP response' do
      ...
      it 'is expected not to succeed' do
        expect { subject }.to raise_error Thingsboard::DeviceApi::UnexpectedResponseCode
      end

      it 'tracks the request and the according error with configured trackers' do
        expect(request_tracker).to receive(:call).with(expected_action)
        expect(error_tracker).to receive(:call).with(expected_action, response_status)
        expect { subject }.to raise_error Thingsboard::DeviceApi::UnexpectedResponseCode
      end
      ...
      context 'when request failed with http-status 403' do
        ...
        it 'is expected to raise Thingsboard::Api::Forbidden' do
          expect { subject }.to raise_error Thingsboard::DeviceApi::Forbidden
        end
      end
